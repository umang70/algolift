...
const ProblemDescription: React.FC<ProblemDescriptionProps> = ({ problem, _solved }) => {
	...
	const handleLike = async () => {
		...
		await runTransaction(firestore, async (transaction) => {
			...
			// Ensure user document exists with expected structure.
			let userData: any;
			if (!userDoc.exists()) {
				userData = {
					uid: user!.uid,
					...
					createdAt: Date.now(),
					updatedAt: Date.now(),
					likedProblems: [] as string[],
					dislikedProblems: [] as string[],
					solvedProblems: [] as string[],
					starredProblems: [] as string[],
				};
			...
			const userLikedProblems: string[] = userData.likedProblems ?? [];
			const userDislikedProblems: string[] = userData.dislikedProblems ?? [];

			let problemData: any = problemDoc.data();
			if (!problemData) {
				const meta = mockProblems.find((p) => p.id === problem.id);
				problemData = {
					title: meta?.title ?? problem.title,
					difficulty: meta?.difficulty ?? "Easy",
					category: meta?.category ?? "Array",
					order: meta?.order ?? 0,
					likes: 0,
					dislikes: 0,
					videoId: meta?.videoId ?? "",
				};
				transaction.set(problemRef, problemData);
			}
	...
	const handleStar = async () => {
		...
		try {
			...
			if (!userSnap.exists()) {
				// Initialize a minimal user document so that array updates work.
				await updateDoc(userRef, {
					...
					// If updateDoc fails because the document truly doesn't exist, create it.
